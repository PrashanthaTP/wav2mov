...
from wav2mov.models.wav2mov_3 import Wav2MovBW
...
def train_model(options, hparams, config, logger):
    ...
    logger.info(
        f'option : num_videos : {options.num_videos} | mean : {mean} std: {std} | stride :{stride}')
    ...
    NUM_VIDEOS = options.num_videos if options.num_videos is not None else len(
        train_dl)
    ################################
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, sample in enumerate(train_dl):
            ...
            model.set_condition(still_image,num_frames=limit)
            ...
            logger.debug(
                f'audio shape : {audio.shape} | video shape : {video.shape}')
        ...
        logger.info(
            f'[Epoch {epoch+1}/{num_epochs}] {epoch_duration:0.2f} seconds or {epoch_duration/60:0.2f} minutes')
