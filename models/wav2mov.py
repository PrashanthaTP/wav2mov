...
class Wav2MovBW(TemplateModel):
    ...
    def __match_num_frames(self,frames,ref_shape):
        batch_size,num_frames,channels,height,width = ref_shape
        self.ref_frames = self.ref_frames.repeat((1,num_frames,1,1,1))
        self.ref_frames = self.ref_frames.reshape((batch_size*num_frames,channels,height,width)) 
        
    def set_input(self,audio,audio_frames,video,ref_frames):
        self.audio = audio
        self.audio_frames = audio_frames
        self.video = video    
        self.ref_frames = ref_frames
        self.__match_num_frames(self.ref_frames,self.video.shape)
    ...
    #     self.audio_frames = batch['audio_frames']
    #     self.video = batch['video']
    #     self.ref_frames = batch['ref_frames']
...
class Wav2MovTemplate(TemplateModel):
    ...
    def backward_sync(self,exclude_fake=False):
        ...
        with amp.autocast():
            ...
            loss_sync /= self.accumulation_steps
        self.scaler.scale(loss_sync).backward()
        return {'sync':(loss_ret,self.audio_frames.shape[0])}
...
class Wav2Mov(TemplateModel):
    ...
    def add_fake_frames(self,frames,target_shape):
        batch_size,frames,*extra = target_shape
        frames = frames.reshape(batch_size,frames,*extra)#from B*F,C,H,W to B,F,C,H,W
    ...
    def __get_sub_batch(self,fraction=2):
        ...
        for i in range(fraction):
            end_fraction = int((1/num_frames)*(i+1))
            batch = {}
            
            batch['real_video_frames'] = self.video[:,start_fraction:end_fraction,:,:,:]
            
            ref_video_frames = self.get_ref_frames(batch['real_video_frames'])
    ...
    def __optimize(self,exclude_fake):     
        losses = {'gen':(0.0,0),
                  ...
                  'sync':(0.0,0),
                  'seq':(0.0,0)}       
        for sub_batch in self.__get_sub_batch():
            self.model.set_input(sub_batch)
            loss_id = self.model.optimize_id()
            for name,(loss,n) in loss_id.items(0):
    ...
    def optimize(self,state):
        ...
        losses = self.__optimize(epoch<self.hparams['pre_learning_epoch'])
