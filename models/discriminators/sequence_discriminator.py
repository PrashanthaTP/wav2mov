...
class SequenceDiscriminator(BaseModel):
    def __init__(self,hparams):
        ...
        cnn = nn.ModuleList([Conv2dBlock(chs[i],chs[i+1],kernel,stride,padding,use_norm=True,use_act=True,act=nn.LeakyReLU(0.2)) for i in range(len(chs)-2)])
        cnn.append(Conv2dBlock(chs[-2],chs[-1],kernel,stride,padding,use_norm=False,use_act=True,act=nn.Tanh()))
