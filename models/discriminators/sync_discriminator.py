...
class SyncDiscriminator(BaseModel):
    def __init__(self, hparams,config):
        ...
        ##############################################################
        # Audio encoding
        #5 frames of 666 :3330 + 666*4 = 5994
        # size     : 3330-> 
        # k,s,p          ->(666,50) 
        # channels :   1 -> 64
        ################################################################
        ...
        self.audio_encoder = nn.Sequential(
            Conv1dBlock(1,32,666,1,0,use_norm=True,use_act=True,act=nn.ReLU()),#5994=>5328,
            
            Conv1dBlock(32,64,4,2,padding=padding_42,use_norm=True,use_act=True,act=nn.ReLU()),#5328=>2664,
            Conv1dBlock(64,64,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            Conv1dBlock(64,64,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            
            Conv1dBlock(64,128,4,2,padding=padding_42,use_norm=True,use_act=True,act=nn.ReLU()),#2664=>1332
            Conv1dBlock(128,128,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            Conv1dBlock(128,128,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            
            Conv1dBlock(128,256,3,2,padding=padding_42,use_norm=True,use_act=True,act=nn.ReLU()),#1332=>666
            Conv1dBlock(256,256,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            Conv1dBlock(256,256,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            
            Conv1dBlock(256,512,4,2,padding=padding_42,use_norm=True,use_act=True,act=nn.ReLU()),#666=>333
            Conv1dBlock(512,512,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
            Conv1dBlock(512,512,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU(),residual=True),
        
            Conv1dBlock(512,512,3,2,padding=padding_32,use_norm=True,use_act=True,act=nn.ReLU()),#((333-3+2)/2)+1=167
            Conv1dBlock(512,512,3,2,padding=padding_32,use_norm=True,use_act=True,act=nn.ReLU()),#((167-3+2)/2)+1=84
            Conv1dBlock(512,512,4,2,padding=padding_42,use_norm=True,use_act=True,act=nn.ReLU()),#((84-4+2)/2)+1=42
            Conv1dBlock(512,512,3,1,padding=padding_31,use_norm=True,use_act=True,act=nn.ReLU()),
            
            ) 
        self.audio_fc = nn.Sequential(
            nn.Linear(42*512,512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512,256),
            nn.ReLU()
    ...
    def forward(self, audio_frames, video_frames):
        ...
        # if i<4:
        #   save_series(video_frames[0].unsqueeze(1),self.config,i)
        #   i+=1
